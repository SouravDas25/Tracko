...
class _DailyTransactionViewState
  ...
  _DailyTransactionViewState();

  @override
  void didUpdateWidget(covariant DailyTransactionView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (SettingUtil.currentMonth != selectedMonth) {
  ...
  Future<void> _goToPreviousMonth() async {
    if (_isProgrammaticLoading) {
      AppLog.d('[DailyTransactionView] skip prev month: already loading');
      return;
    }
    if (mounted) {
      setState(() {
  ...
  Future<void> _goToNextMonth() async {
    if (_isProgrammaticLoading) {
      AppLog.d('[DailyTransactionView] skip next month: already loading');
      return;
    }
    if (mounted) {
      setState(() {
  ...
  Future<void> _selectMonth() async {
    if (_isProgrammaticLoading) {
      AppLog.d('[DailyTransactionView] skip month picker: already loading');
      return;
    }
  ...
  asyncLoad() async {
    AppLog.d(
        '[DailyTransactionView] asyncLoad start embedded=${widget.embedded} initialAccountIds=${widget.initialAccountIds}');
    try {
      ...
      selectedMonth = SettingUtil.currentMonth;
      AppLog.d('[DailyTransactionView] selectedMonth=$selectedMonth');

      await refresh();
      this.loadCompleteView();
      AppLog.d('[DailyTransactionView] asyncLoad complete');
    } catch (e) {
  ...
  Future<void> refresh() async {
    AppLog.d('[DailyTransactionView] refresh start month=$selectedMonth');
    try {
      ...
      if (this.mounted) {
        setState(() {
          ...
          if (_isProgrammaticLoading) {
            AppLog.d(
                '[DailyTransactionView] clearing programmatic loader (success)');
          }
          _isProgrammaticLoading = false;
      ...
      if (mounted) {
        setState(() {
          if (_isProgrammaticLoading) {
            AppLog.d(
                '[DailyTransactionView] clearing programmatic loader (error)');
          }
          _isProgrammaticLoading = false;
        });
        Navigator.pushReplacementNamed(context, '/login');
  ...
  initTransactionData({bool isRefresh = true}) async {
    ...
    AppLog.d(
        '[DailyTransactionView] fetched transactions count=${newTransactions.length} total=${transactions.length}');
    ...
    if (isRefresh) {
      ...
      AppLog.d(
          '[DailyTransactionView] totals previous=$previousMonthAmount balance=$totalAmount');
    }
  }
