...
from utils import encode_dataset, flatten, iter_data, find_trainable_variables, convert_gradient_to_tensor, shape_list, \
...
if __name__ == '__main__':
    ...
    shapes = json.load(open(str(pathlib.Path(__file__).parent / 'model/params_shapes.json')))
    offsets = np.cumsum([np.prod(shape) for shape in shapes])
    init_params = [np.load(str(pathlib.Path(__file__).parent / 'model/params_{}.npy'.format(n))) for n in range(10)]
