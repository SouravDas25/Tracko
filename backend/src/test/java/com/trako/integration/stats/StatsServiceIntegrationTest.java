...
public class StatsServiceIntegrationTest extends BaseIntegrationTest {
    ...
    public void customRangeShortReturnsDailySeries() {
        ...
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 10.0, date(2026, 1, 2));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 20.0, date(2026, 1, 5));
    ...
    public void customRangeLongReturnsMonthlySeries() {
        ...
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 10.0, date(2026, 1, 15));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 20.0, date(2026, 2, 15));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 30.0, date(2026, 3, 15));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 40.0, date(2026, 4, 1)); // Transaction on the last day
    ...
    public void monthlyStatsBuildsMonthlySeriesAndSkipsInvalidCategoryIds() {
        ...
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 10.0, date(2026, 1, 5));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 20.0, date(2026, 2, 5));
        saveTx(travel.getId(), TransactionDbType.DEBIT, 1, 5.0, date(2026, 2, 6));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, null, date(2026, 2, 7));
    ...
    public void monthlySeriesCoversAllMonthLabels() {
        ...
        for (int m = 1; m <= 12; m++) {
            saveTx(food.getId(), TransactionDbType.DEBIT, 1, 1.0, date(2026, m, 2));
        }
    ...
    public void yearlyStatsBuildsYearlySeries() {
        ...
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 10.0, date(2025, 12, 31));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 20.0, date(2026, 1, 1));
    ...
    public void statsHasEmptySeriesWhenNoMatchingKindTransactions() {
        ...
        saveTx(food.getId(), TransactionDbType.CREDIT, 1, 50.0, date(2026, 1, 7));
        saveTx(food.getId(), TransactionDbType.DEBIT, null, 50.0, date(2026, 1, 7));
    ...
    public void categoryStatsWeeklyFiltersByCategoryAndComputesTotalForCurrentPeriod() {
        ...
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 40.0, date(2026, 1, 7));
        saveTx(travel.getId(), TransactionDbType.DEBIT, 1, 999.0, date(2026, 1, 7));
        saveTx(food.getId(), TransactionDbType.DEBIT, 1, 500.0, date(2025, 12, 15));
    ...
    private void saveExpense(Long categoryId, double amount, Date d) {
        saveTx(categoryId, TransactionDbType.DEBIT, 1, amount, d);
    }

    private void saveTx(Long categoryId, TransactionDbType transactionType, Integer isCountable, Double amount, Date d) {
        Transaction t = newTx(categoryId, transactionType, isCountable, amount, d);
        transactionWriteService.saveForUser(user.getId(), t);
    }

    private Transaction newTx(Long categoryId, TransactionDbType transactionType, Integer isCountable, Double amount, Date d) {
